...
namespace VisionCore.appAutoStartUp
{
    class SJappAutoStartUp
    {
        /// <summary>
        /// 快捷方式名称-任意自定义
        /// </summary>
        private const string QuickName = "VM.Start";
        ...
        public void SetMeAutoStart(bool onOff = true)
        {
            if (onOff) //开机启动
            {
                ...
                {
                    CreateShortcut(systemStartPath, QuickName, appAllPath, "智能装备");
                }
            }
            else //开机不启动
            {
        ...
        )
        {
            try
            {
                ...
                string shortcutPath = Path.Combine(
                    directory,
                    string.Format("{0}.lnk", shortcutName)
                ); //合成路径
                WshShell shell = new IWshRuntimeLibrary.WshShell();
                IWshShortcut shortcut = (IWshRuntimeLibrary.IWshShortcut)
                    shell.CreateShortcut(shortcutPath); //创建快捷方式对象
                shortcut.TargetPath = targetPath; //指定目标路径
                shortcut.WorkingDirectory = Path.GetDirectoryName(targetPath); //设置起始位置
                shortcut.WindowStyle = 1; //设置运行方式，默认为常规窗口
                shortcut.Description = description; //设置备注
                shortcut.IconLocation = string.IsNullOrWhiteSpace(iconLocation)
                    ? targetPath
                    : iconLocation; //设置图标路径
        ...
        private List<string> GetQuickFromFolder(string directory, string targetPath)
        {
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                if (tempStr.ToLower() == targetPath.ToLower())
                {
